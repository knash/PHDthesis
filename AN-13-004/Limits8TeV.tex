...
\end{center}
\caption{Nuisance parameters after the fit.  This the nominal value found for the nuisance parameter after the fit in units of input sigma.The numbers listed under sample specify $\wpr_R$ signal Monte Carlo mass.}
...
Where $\sigma_{W`_{ij}}$ refers to the cross-section for right, left or mixed-coupling samples and $\sigma_{single top}$ is the 
...
%comparing the summed $\wpr_{R}$ and $\wpr_{L}$ $M_{tb}$ spectrum to this spectrum in $\wpr_{LR}$.  
